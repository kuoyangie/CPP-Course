...
/*	-------------------------------------------------------------------------
*	This function will RECURSIVELY print out the nodes in the list backwards*
*	If the list is empty, will print out list contents 0		            *
*	Otherwise, print out the address, value, and the node's next address.	*
*	-------------------------------------------------------------------------
*/
void IntList::internalBackwardPrint(ListNode* node){	
	if (node == nullptr) return;
	else if (node) {
		internalBackwardPrint(node->next);
		cout << node << ":\tvalue =\t" << node->value << "\tnext =" << node->next << endl;
	}
}

/*	-------------------------------------------------------------------------
*	This is the wrapper function of internalBackwardPrint() function   		*
...
*	-------------------------------------------------------------------------
*/
void IntList::printBackWards()
{	
	cout << "BACKWARDS recursive print of list, head = " << head << ".\n";
	internalBackwardPrint(head);
...
/*	-------------------------------------------------------------------------
*	This function is the wrapper function of internalLengthRecusion	function*
*	-------------------------------------------------------------------------
*/
void IntList::lengthRecursion()
{
	int numberOfNodes = 0;
	numberOfNodes = internalLengthRecursion(head);

	cout << "The number of " << (numberOfNodes > 1 ? "nodes " : "node ")
				<< "in list, head = " << head << " count = " << numberOfNodes << ".\n" << endl;
...
*	-------------------------------------------------------------------------
*/
int IntList::internalLengthRecursion(ListNode *node)
{
	if (node == nullptr) {
		return 0;
	}
		...
		node = node->next;
		return 1 + internalLengthRecursion(node);;
	}
}

/*	-------------------------------------------------------------------------
*	This function will traverse the linked list and sum up                  *
*	the value of each element during the process using recursive method	    *
*	Return data type: int						      						*
*	-------------------------------------------------------------------------
*/
int IntList::internalTotalRecursion(ListNode* node)
{
	if (node == nullptr) return 0;
	else if (node) {
		return node->value + internalTotalRecursion(node->next);
	}
}

/*	-------------------------------------------------------------------------
*	This function is the wrapper function of internalTotalRecusion function *
*	-------------------------------------------------------------------------
*/
void IntList::totalRecursion()
{
	int nodeSumRecursion = 0;
	nodeSumRecursion = internalTotalRecursion(head);

	cout << "Total of all value fields in list, head = " << head << ", total = " << nodeSumRecursion << "\n" << endl;
}
