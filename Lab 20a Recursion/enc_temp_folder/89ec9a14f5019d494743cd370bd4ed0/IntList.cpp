...
{
	int numberOfNodes = 0;
	numberOfNodes = internalLengthRecursion(head, numberOfNodes);
...
int IntList::internalLengthRecursion(ListNode *node, int numberOfNodes)
{
	if (node == nullptr) {
		return 0;
	}
		...
		node = node->next;
		return internalLengthRecursion(node, numberOfNodes++);;
	}
}
